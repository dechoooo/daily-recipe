...
class _RecipeEditPageState extends State<RecipeEditPage> with SingleTickerProviderStateMixin {
    ...
    try {
      ...
      await db.transaction((txn) async {
        ...
        if (widget.recipe?.id == null) {
          // 插入新菜谱
          recipeId = await txn.insert('recipes', recipe.toMap());
          // 同步标签库
          await _syncTagsInTransaction(txn, recipe);
        } else {
          // 更新现有菜谱
          await txn.update('recipes', recipe.toMap(),
        ...
        for (int i = 0; i < _variants.length; i++) {
          ...
          v.recipeId = recipeId;
          v.sortOrder = i;
          await txn.insert('variants', v.toMap());
        }
      });
  ...
  /// 在事务中同步标签库
  Future<void> _syncTagsInTransaction(Transaction txn, Recipe recipe) async {
