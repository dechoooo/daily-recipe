...
class DatabaseService {
  ...
  /// 搜索菜谱（关键词 + 分类筛选叠加）
  /// [keyword] 搜索关键词（菜名、食材、步骤），自动 trim
  /// [categoryId] 可选，按分类筛选
  Future<List<Recipe>> searchRecipes({String keyword = '', int? categoryId}) async {
    ...
    if (keyword.isNotEmpty) {
      // 在 name、ingredients、steps、tags 中搜索
      final like = '%$keyword%';
      conditions.add('(name LIKE ? OR ingredients LIKE ? OR steps LIKE ? OR tags LIKE ?)');
